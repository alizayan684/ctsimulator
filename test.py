...
class CTMathEngine:
    ...
    def generate_system_matrix_exact(N, num_angles, num_detectors, progress_callback=None, update_callback=None):
        """Calculates the exact chord lengths of 2D rays intersecting square pixels (Siddon-equivalent footprint)."""
    ...
    def simulate_acquisition_physics(sinogram_clean, mA, kVp):
        """Simulates physics of acquisition including Polychromatic Beam Hardening and Poisson noise."""
        ...
        I_noisy = np.random.poisson(I_mean).astype(np.float64)
...
DARK_STYLESHEET = """
QMainWindow, QWidget { background-color: #0B0C0E; color: #E2E8F0; font-family: "Segoe UI", "SF Pro Display", "Helvetica Neue", Arial, sans-serif; font-size: 13px; }
QSplitter::handle { background-color: #1E2128; width: 1px; }
QScrollArea { border: none; background-color: #0B0C0E; }
QScrollArea > QWidget > QWidget { background-color: #0B0C0E; }
QScrollBar:vertical { background: #0B0C0E; width: 6px; border: none; }
QScrollBar::handle:vertical { background: #252830; border-radius: 3px; min-height: 20px; }
QScrollBar::handle:vertical:hover { background: #3B82F6; }
QScrollBar::add-line:vertical, QScrollBar::sub-line:vertical { height: 0px; }
QScrollBar:horizontal { height: 0px; }
QGroupBox { background-color: #13151A; border: 1px solid #252830; border-radius: 10px; margin-top: 20px; padding: 14px 10px 10px 10px; }
QGroupBox::title { subcontrol-origin: margin; subcontrol-position: top left; left: 10px; top: 0px; padding: 2px 8px; background-color: #13151A; color: #3B82F6; border-radius: 4px; font-size: 10px; font-weight: 700; letter-spacing: 1.2px; text-transform: uppercase; }
QLabel { color: #CBD5E1; background: transparent; font-size: 13px; }
QPushButton { background-color: #1C1F27; color: #CBD5E1; border: 1px solid #2D3139; border-radius: 7px; padding: 7px 10px; font-size: 12px; font-weight: 500; min-height: 30px; }
QPushButton:hover { background-color: #252A35; border-color: #3B82F6; color: #E2E8F0; }
QPushButton:pressed { background-color: #191D25; border-color: #2563EB; }
QPushButton:disabled { background-color: #111318; border-color: #1C1F27; color: #3A4252; }
QPushButton#btn_generate_sys { background: qlineargradient(x1:0, y1:0, x2:1, y2:0, stop:0 #1D4ED8, stop:1 #2563EB); color: #FFFFFF; border: none; border-radius: 7px; font-weight: 600; font-size: 12px; padding: 8px 10px; min-height: 32px; }
QPushButton#btn_generate_sys:hover { background: qlineargradient(x1:0, y1:0, x2:1, y2:0, stop:0 #2563EB, stop:1 #3B82F6); border: none; }
QPushButton#btn_generate_sys:pressed { background: qlineargradient(x1:0, y1:0, x2:1, y2:0, stop:0 #1E40AF, stop:1 #1D4ED8); }
QPushButton#btn_generate_sys:disabled { background: #1A2340; color: #3A5080; border: none; }
QPushButton#btn_reconstruct { background: qlineargradient(x1:0, y1:0, x2:1, y2:0, stop:0 #065F46, stop:1 #059669); color: #ECFDF5; border: none; border-radius: 7px; font-weight: 700; font-size: 13px; padding: 10px 10px; min-height: 38px; letter-spacing: 0.3px; }
QPushButton#btn_reconstruct:hover { background: qlineargradient(x1:0, y1:0, x2:1, y2:0, stop:0 #059669, stop:1 #10B981); border: none; }
QPushButton#btn_reconstruct:pressed { background: qlineargradient(x1:0, y1:0, x2:1, y2:0, stop:0 #064E3B, stop:1 #065F46); }
QPushButton#btn_reconstruct:disabled { background: #0E2018; color: #1D4433; border: none; }
QTabWidget::pane { background-color: #13151A; border: 1px solid #252830; border-radius: 10px; top: -1px; }
QTabBar { background: transparent; }
QTabBar::tab { background-color: #0B0C0E; color: #4A5568; border: 1px solid #252830; border-bottom: none; border-top-left-radius: 8px; border-top-right-radius: 8px; padding: 8px 18px; margin-right: 3px; font-size: 12px; font-weight: 500; min-width: 130px; }
QTabBar::tab:selected { background-color: #13151A; color: #3B82F6; border-color: #252830; border-bottom-color: #13151A; font-weight: 700; }
QTabBar::tab:hover:!selected { background-color: #0F1117; color: #94A3B8; border-color: #2D3448; }
QSpinBox, QDoubleSpinBox { background-color: #181B22; color: #E2E8F0; border: 1px solid #2D3139; border-radius: 6px; padding: 5px 6px 5px 8px; font-size: 12px; font-family: "JetBrains Mono", "Cascadia Code", "Consolas", monospace; min-height: 28px; selection-background-color: #3B82F6; }
QSpinBox:hover, QDoubleSpinBox:hover { border-color: #3B82F6; background-color: #1C1F28; }
QSpinBox:focus, QDoubleSpinBox:focus { border-color: #3B82F6; background-color: #1A2030; }
QSpinBox::up-button, QDoubleSpinBox::up-button { subcontrol-origin: border; subcontrol-position: top right; width: 20px; border-left: 1px solid #2D3139; border-bottom: 1px solid #2D3139; border-top-right-radius: 5px; background-color: #1E2128; }
QSpinBox::down-button, QDoubleSpinBox::down-button { subcontrol-origin: border; subcontrol-position: bottom right; width: 20px; border-left: 1px solid #2D3139; border-top: 1px solid #2D3139; border-bottom-right-radius: 5px; background-color: #1E2128; }
QSpinBox::up-button:hover, QDoubleSpinBox::up-button:hover, QSpinBox::down-button:hover, QDoubleSpinBox::down-button:hover { background-color: #3B82F6; }
QSpinBox::up-arrow, QDoubleSpinBox::up-arrow { image: none; border-left: 4px solid transparent; border-right: 4px solid transparent; border-bottom: 5px solid #94A3B8; }
QSpinBox::down-arrow, QDoubleSpinBox::down-arrow { image: none; border-left: 4px solid transparent; border-right: 4px solid transparent; border-top: 5px solid #94A3B8; }
QComboBox { background-color: #181B22; color: #E2E8F0; border: 1px solid #2D3139; border-radius: 6px; padding: 5px 32px 5px 10px; font-size: 12px; min-height: 28px; selection-background-color: #3B82F6; }
QComboBox:hover { border-color: #3B82F6; background-color: #1C1F28; }
QComboBox:focus { border-color: #3B82F6; background-color: #1A2030; outline: none; }
QComboBox:on { border-color: #3B82F6; border-bottom-left-radius: 0px; border-bottom-right-radius: 0px; background-color: #1A2030; }
QComboBox::drop-down { subcontrol-origin: padding; subcontrol-position: top right; width: 28px; border-left: 1px solid #2D3139; border-top-right-radius: 6px; border-bottom-right-radius: 6px; background-color: #1E2128; }
QComboBox::drop-down:hover { background-color: #3B82F6; }
QComboBox::down-arrow { image: none; border-left: 5px solid transparent; border-right: 5px solid transparent; border-top: 6px solid #94A3B8; }
QComboBox::down-arrow:on { border-top: none; border-bottom: 6px solid #3B82F6; }
QComboBox QAbstractItemView { background-color: #181B22; color: #E2E8F0; border: 1px solid #3B82F6; border-top: none; border-bottom-left-radius: 6px; border-bottom-right-radius: 6px; selection-background-color: #1D3A6E; selection-color: #FFFFFF; outline: none; padding: 4px 0px; }
QComboBox QAbstractItemView::item { padding: 7px 12px; min-height: 28px; border: none; color: #CBD5E1; }
QComboBox QAbstractItemView::item:hover { background-color: #1E2840; color: #E2E8F0; }
QComboBox QAbstractItemView::item:selected { background-color: #1D3A6E; color: #FFFFFF; }
QProgressBar { background-color: #181B22; border: 1px solid #252830; border-radius: 4px; height: 6px; text-align: center; color: transparent; font-size: 1px; }
QProgressBar::chunk { background: qlineargradient(x1:0, y1:0, x2:1, y2:0, stop:0 #2563EB, stop:1 #3B82F6); border-radius: 4px; }
QLabel#lbl_metrics { background-color: #13151A; border: 1px solid #252830; border-radius: 8px; padding: 8px 16px; color: #10B981; font-family: "JetBrains Mono", "Cascadia Code", "Consolas", monospace; font-size: 13px; font-weight: 600; letter-spacing: 0.5px; }
QLabel#lbl_roi_stats { background-color: #13151A; border: 1px solid #252830; border-radius: 8px; padding: 12px 14px; color: #10B981; font-family: "JetBrains Mono", "Cascadia Code", "Consolas", monospace; font-size: 11px; line-height: 1.6; }
QLabel#lbl_status { color: #4A5568; font-size: 11px; font-style: italic; padding: 3px 2px; background: transparent; }
QToolTip { background-color: #1E2128; color: #E2E8F0; border: 1px solid #3B82F6; border-radius: 5px; padding: 5px 8px; font-size: 12px; }
"""
...
def _field(label_text, widget):
    ...
    lbl.setStyleSheet("color: #4A5568; font-size: 10px; font-weight: 600; letter-spacing: 0.6px; background: transparent;")
...
class CTApp(QMainWindow):
    def __init__(self):
        ...
        # Initialize UI before using any UI-dependent values
        self.init_ui()
    ...
    def init_ui(self):
        ...
        title_lbl.setStyleSheet("color: #E2E8F0; font-size: 14px; font-weight: 700; letter-spacing: 0.4px; padding: 2px 0px 10px 2px; background: transparent; border-bottom: 1px solid #1E2128;")
        ...
        self.spin_grid = QSpinBox(); self.spin_grid.setRange(16, 256); self.spin_grid.setValue(64)
        self.spin_angles = QSpinBox(); self.spin_angles.setRange(10, 720); self.spin_angles.setValue(90)
        self.spin_detectors = QSpinBox(); self.spin_detectors.setRange(16, 512); self.spin_detectors.setValue(90)
        ...
        self.spin_ma = QSpinBox(); self.spin_ma.setRange(1, 1000); self.spin_ma.setValue(200)
        self.spin_kvp = QSpinBox(); self.spin_kvp.setRange(40, 150); self.spin_kvp.setValue(120)
        ...
        self.spin_iters = QSpinBox(); self.spin_iters.setRange(1, 1000); self.spin_iters.setValue(20)
        self.spin_relax = QDoubleSpinBox(); self.spin_relax.setRange(0.01, 2.0); self.spin_relax.setValue(0.1); self.spin_relax.setSingleStep(0.1)
        self.spin_tol = QDoubleSpinBox(); self.spin_tol.setRange(0.0001, 0.1); self.spin_tol.setValue(0.001); self.spin_tol.setDecimals(4)
        ...
        self.combo_cmap = QComboBox(); self.combo_cmap.addItems(["gray", "bone", "viridis", "inferno"])
        ...
        self.sim_tab = QWidget(); self.sim_tab.setStyleSheet("background-color: #13151A;")
        ...
        self.analysis_tab = QWidget(); self.analysis_tab.setStyleSheet("background-color: #13151A;")
        ...
        self.canvas_analysis.setStyleSheet("background-color: #13151A; border: 1px solid #252830; border-radius: 8px;")
        self.ax_analysis_img = self.fig_analysis.add_subplot(111)
        self.ax_analysis_img.set_title("Draw Line for Profile (Click & Drag)", color='#CBD5E1', fontsize=10)
        ...
        self.canvas_profile.setStyleSheet("background-color: #13151A; border: 1px solid #252830; border-radius: 8px;")
    ...
    def load_custom_phantom(self):
        filepath, _ = QFileDialog.getOpenFileName(self, "Load Custom Phantom", "", 
    ...
    def on_matrix_generated(self, A):
        ...
        self.lbl_status.setText("Exact System Matrix ready. Simulating physics…")
    ...
    def on_mouse_press(self, event):
        ...
        self.profile_line = Line2D([event.xdata, event.xdata], [event.ydata, event.ydata], color='#3B82F6', lw=1.5, linestyle='--')
        self.ax_analysis_img.add_line(self.profile_line)
        self.canvas_analysis.draw()
    ...
    def on_mouse_drag(self, event):
        ...
        self.profile_line.set_data([self.line_points[0][0], self.line_points[1][0]], [self.line_points[0][1], self.line_points[1][1]])
        self.canvas_analysis.draw()
    ...
    def extract_line_profile(self):
        ...
        for spine in self.ax_profile.spines.values(): spine.set_edgecolor('#252830')
